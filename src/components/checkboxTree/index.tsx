...
export const CheckboxTree: Hierarchy = React.forwardRef((props, ref) => {
  const {
    data,
    textField,
    childField,
  ...
  useEffect(() => {
    const selectedItem = data.map((e: any) => {
      delete e.parent;
      delete e.childs;
      delete e.show;
      delete e.tick;
  ...
  const _renderIcon = (status: boolean) => {
    if (status) {
      if (checkIcon) {
        ...
        return <Ionics name='ios-checkbox-outline' size={iconSize} color={iconColor} />
      }
      ...
      if (unCheckIcon) {
        ...
        return <Ionics name='stop-outline' size={iconSize} color={iconColor} />
      }
    }
  };

  const renderList = (item: any, childs: any, index: number) => {
  console.log("data[childField]",data[0]);
        ...
        {renderItem ? renderItem({ item: item, isSelect: item.tick, isOpen: item.show, onOpen: () => { showChild(item) }, onClose: () => { showChild(item) }, onSelect: () => { !item.tick ? onTick(item) : onUnTick(item) } }) : <View style={styles.rowItem}>
          {childs && childs.length > 0 ? (
            <TouchableOpacity
              ...
              {item.show ? openIcon ? openIcon : <Ionics name='ios-remove' size={iconSize} color={iconColor} /> : closeIcon ? closeIcon : <Ionics name='add-outline' size={iconSize} color={iconColor} />}
            </TouchableOpacity>
